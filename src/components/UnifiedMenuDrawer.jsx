  ...
  const suggestions = (usersList || [])
    .filter(
      (u) =>
        ...
        !pending.some((r) => r.fromUid === u.uid)
    )
    .slice(0, 5);
  ...
  return (
    <div className="mt-5 relative">
            ...
            return (
              ...
              >
                <div
                  ...
                  className="w-9 h-9 rounded-full bg-slate-200 flex-shrink-0 overflow-hidden border-2 border-amber-300 cursor-pointer hover:opacity-80"
                >
                  {sender?.avatar ? (
                ...
                <div className="flex gap-1 flex-shrink-0">
                  ...
                  <button
                    ...
                    className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors cursor-pointer text-xs font-black ${
                      isDarkMode
          ...
          <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2 px-1">
            {suggestions.map((s) => {
              ...
              return (
                <div
                  key={s.uid}
                  className={`flex flex-col items-center w-[85px] p-2.5 rounded-2xl border flex-shrink-0 ${
                    isDarkMode
                ...
                >
                  ...
                  >
                    {s.avatar ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center font-black text-slate-500">
                        {(s.nickname || s.email || 'U').charAt(0).toUpperCase()}
                      </div>
                    )}
                  </div>

                  <p
                    className={`text-[10px] font-bold truncate w-full text-center mb-2 ${
                  ...
                  >
                    {isSent ? 'Đã gửi' : (
                      <>
                        <FiUserPlus className="text-[10px]" /> Thêm
                      </>
                    )}
                  </button>
                </div>
      ...
      {myFriends.length === 0 && pending.length === 0 ? (
        <div
          className={`text-center py-4 rounded-2xl border border-dashed ${
            isDarkMode
      ...
      {viewingProfile && (
        <div
          className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in"
          onClick={() => setViewingProfile(null)}
        >
          <div
            className={`rounded-2xl p-6 w-full max-w-xs shadow-2xl relative ${
          ...
          >
            ...
            <div className="flex flex-col items-center mt-2">
              <img
                src={viewingProfile.avatar || 'https://via.placeholder.com/150'}
              ...
              <h3 className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
                {viewingProfile.nickname || viewingProfile.email?.split('@')[0]}
              </h3>

              <p
                className={`text-[11px] font-medium mb-5 px-3 py-1 rounded-full mt-2 ${
              ...
              <div className="flex w-full gap-2">
                {friendsList.includes(viewingProfile.uid) ? (
                  <button
                    onClick={() => {
                      ...
                      document.dispatchEvent(
                        new CustomEvent('trimi:open-chat', { detail: viewingProfile })
                      );
                    }}
                  ...
                  >
                    {sentRequests.includes(viewingProfile.uid) ? 'Đã gửi yêu cầu' : (
                      <>
                        <FiUserPlus /> Kết bạn
                      </>
                    )}
...
export default function UnifiedMenuDrawer({
  ...
  return (
    <div className="fixed inset-0 z-[100000] flex justify-end">
      ...
      <div
        className={`relative w-full md:w-[320px] h-full flex flex-col shadow-2xl md:border-l overflow-y-auto transform transition-transform duration-300 ${
          isDarkMode
            ? 'bg-[#111111] md:border-slate-800 text-white'
            : 'bg-white md:border-slate-200 text-slate-900'
        }`}
      >
        <div
        ...
        <div className="flex-grow flex flex-col px-5 py-4 gap-1.5">
          {littleTrimiConfig && updateLittleTrimiConfig && (
            <div
              className={`md:hidden mt-4 p-4 rounded-2xl border ${
                isDarkMode
                  ? 'bg-slate-800/50 border-slate-700'
                  : 'bg-slate-50 border-slate-200'
              }`}
            >
              <p className="text-[11px] font-black text-sky-500 uppercase tracking-widest mb-3 flex items-center gap-2"></p>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className={`w-full border rounded-xl px-2 py-2 outline-none focus:border-sky-500 text-xs font-bold ${
                      isDarkMode ? 'bg-black/30 border-white/10' : 'bg-white border-black/10'
                    }`}
                  >
          ...
          <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 mt-2 ml-1">
            Giao diện & Cài đặt
          </p>
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className={`flex flex-col items-center justify-center p-3 rounded-2xl font-bold transition-colors border ${
                isDarkMode
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-xs font-bold border transition-colors ${
                      isDarkMode
                        ...
                        : 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-sky-50 hover:border-sky-300 hover:text-sky-600'
                    }`}
                  >
                    {item.label}
                  </button>
                  ...
                  item.href ? (
                    <a
                      ...
                      className={`w-10 h-10 rounded-full flex items-center justify-center border transition-colors ${
                        isDarkMode
                          ? 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-sky-500 hover:border-sky-500 hover:text-white'
                          : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-sky-500 hover:border-sky-500 hover:text-white'
                      }`}
                    >
        ...
        <div
          className={`p-4 mt-auto border-t ${
            isDarkMode ? 'border-slate-800 bg-[#111111]' : 'border-slate-100 bg-white'
          }`}
        >
          {isAuthenticated ? (
            <button

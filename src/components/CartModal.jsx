...
export default function CartModal({
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex justify-end pointer-events-none">
      {/* Cart panel — re-enable pointer-events so it's clickable */}
      <div
        className={`relative w-full max-w-[360px] h-full flex flex-col shadow-2xl animate-fade-in-right backdrop-blur-xl border-l pointer-events-auto ${
          isDarkMode
            ? 'bg-[#181512]/60 border-white/10 text-white'
            : 'bg-[#f7f3ed]/70 border-black/10 text-slate-900'
        }`}
      >
        {/* ── HEADER ── */}
            ...
            cart.map((item, index) => (
              ...
              >
                ...
                <div className="flex-1 flex flex-col justify-center">
                  ...
                  <p className="font-black text-sm mt-0.5 text-sky-500">
        ...
        {/* ── FOOTER: PAYMENT MODE + TOTAL + CHECKOUT ── */}
        {cart.length > 0 && (
            ...
            <div className={`flex gap-2 mb-4 p-1.5 rounded-xl border ${isDarkMode ? 'bg-black/40 border-white/10' : 'bg-white/50 border-black/5'}`}>
              <button
                ...
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                  paymentMode === 'deposit'
                    ? isDarkMode ? 'bg-white/20 text-white shadow-sm' : 'bg-white shadow-sm text-slate-900'

import {
  ...
  FiX, FiArchive, FiImage, FiTruck, FiPhone,
} from 'react-icons/fi';
import { doc, setDoc, deleteDoc } from 'firebase/firestore';

// ── Hardcoded shipper info ────────────────────────────────────────────────────
const SHIPPER = {
  name:  'Phạm Vũ Hoàng',
  phone: '0865481225',
  image: '/shipper.jpg',
          ...
          <div className="mt-4 md:mt-0 relative z-20">
            {!isEditingName ? (
              <h2 className="text-3xl md:text-4xl font-black text-slate-900 flex items-center gap-3">
                {nickname}
                <button onClick={() => { setTempName(nickname); setIsEditingName(true); }} className="text-sm text-slate-400 hover:text-sky-500 transition-colors p-1.5 bg-slate-50 rounded-full border border-slate-100" title="Đổi biệt danh">
                  <FiEdit3/>
                </button>
                <button
                  onClick={() => {
      ...
      {myOrders.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          {myOrders.map(order => (
            <div
              key={order.id}
              className={`p-6 rounded-[32px] border shadow-sm flex flex-col gap-5 ${isDarkMode ? 'bg-[#1e293b] border-slate-700' : 'bg-white border-slate-200'}`}
            >
              {/* Top row: info + status */}
              <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
                ...
                <div className="flex flex-col items-start md:items-end gap-3 mt-4 md:mt-0">
                  ...
                  <span className={`px-5 py-2.5 rounded-full text-[11px] font-black uppercase tracking-widest shadow-sm ${
                    order.status.includes('Hoàn thành')
                      ? 'bg-emerald-500 text-white'
                      : order.status.includes('hủy') || order.status.includes('từ chối')
              ...
              {/* ── SHIPPER CARD ── only when status is "Đang giao hàng" ── */}
              {order.status === 'Đang giao hàng' && (
                <div className={`flex items-center gap-4 p-4 rounded-2xl border-2 border-sky-200 ${isDarkMode ? 'bg-sky-500/10' : 'bg-sky-50'}`}>
                  ...
                  <div className="relative flex-shrink-0">
                    <img
                      src={SHIPPER.image}
                      alt={SHIPPER.name}
                      className="w-16 h-16 rounded-full object-cover border-2 border-sky-300 shadow-md"
                      onError={(e) => {
                        // Fallback if shipper.jpg not yet in public folder
                        e.target.style.display = 'none';
                        e.target.nextSibling.style.display = 'flex';
                      }}
                    />
                    {/* Fallback avatar */}
                    <div className="w-16 h-16 rounded-full bg-sky-500 text-white hidden items-center justify-center text-2xl font-black border-2 border-sky-300 shadow-md">
                  ...
                  <div className="flex-1">
                    <p className={`text-[11px] font-black uppercase tracking-widest mb-1 flex items-center gap-1.5 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`}>
                      <FiTruck className="text-sm"/> Shipper đang trên đường giao
                    </p>
                    ...
                    <a
                      ...
                      className="text-sky-500 font-bold text-sm hover:text-sky-600 hover:underline flex items-center gap-1.5 mt-0.5 w-fit"
                    >
                      <FiPhone className="text-xs"/> {SHIPPER.phone}
                    </a>
                  </div>
                  ...
                  <a
                    href={`tel:${SHIPPER.phone}`}
                    className="hidden md:flex items-center gap-2 bg-sky-500 hover:bg-sky-600 text-white px-5 py-2.5 rounded-full font-bold text-sm transition-colors shadow-md flex-shrink-0"
                  >
                    <FiPhone/> Gọi ngay
                  </a>
                </div>
